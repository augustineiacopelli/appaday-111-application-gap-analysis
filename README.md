...
Two sequential Claude API calls. The first reads only the job description and returns structured JSON requirements, each tagged required or preferred, plus any stated salary range and travel percentage. The second reads the resume plus those requirements (never the raw job text again) plus optional role context and an optional ranked strengths list, and returns a gap list, structural findings, and fit questions as JSON. A Print / Save as PDF button appears once results are in, and produces a clean report: input fields, the settings gear, and status chrome are hidden, every gap row is force-expanded so evidence and rewrites are visible without tapping, and a print-only header stamps the report with the date, extracted role, and company. It uses the browser's native print-to-PDF rather than a bundled PDF library, keeping the app dependency-free.
